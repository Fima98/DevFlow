...
import { Collection, Question, Tag, TagQuestion, Vote, Answer, Interaction } from "@/database";
...
export async function createQuestion(
  ...
  const { title, content } = validationResult.params!;
  let { tags } = validationResult.params!;
...
export async function editQuestion(params: EditQuestionParams): Promise<ActionResponse<Question>> {
  ...
  const { title, content, questionId } = validationResult.params!;
  let { tags } = validationResult.params!;
  tags = [...new Set(tags.map((tag) => tag.trim().toLowerCase()))];
  const userId = validationResult.session?.user?.id;
...
export async function getRecommendedQuestions({
  ...
  const interactions = await Interaction.find({
    ...
    actionTarget: "question",
    actionType: { $in: ["view", "upvote", "bookmark", "post"] },
  })
    .sort({ createdAt: -1 })
    .limit(50)
    .lean();
  ...
  const interactedQuestions = await Question.find({ _id: { $in: interactedQuestionIds } }).select(
    "tags"
  );

  const allTags = [
    ...new Set(
      interactedQuestions.flatMap((q) => q.tags.map((tag: Types.ObjectId) => tag.toString()))
    ),
  ];
  ...
  const recommendedQuery: FilterQuery<typeof Question> = {
    ...
    tags: { $in: allTags.map((id) => new Types.ObjectId(id)) },
  };
